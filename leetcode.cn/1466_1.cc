...
class Solution {
    int dfs(unordered_map<int, vector<pair<int, int>>> &edges, int s, int p)
    {
        int num = 0;
        for (const auto &n : edges[s])
        {
            if (n.first == p) continue;
            num += n.second + dfs(edges, n.first, s);
        }
        return num;
    }
public:
    int minReorder(int n, vector<vector<int>> &connections)
    {
        ...
        for (int i = 0; i < n - 1; ++i)
        {
            ...
            edges[s].emplace_back(e, 1);
            edges[e].emplace_back(s, 0);
        }
