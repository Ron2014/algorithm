...
INVALID_MASK = set(["0.0.0.0", "255.255.255.255"])
...
MAP_CLASS = [
    ((  1,  0,0,0,),    (126,255,255,255,), IDX_A,),
    ((128,  0,0,0,),    (191,255,255,255,), IDX_B,),
    ((192,  0,0,0,),    (223,255,255,255,), IDX_C,),
    ((224,  0,0,0,),    (239,255,255,255,), IDX_D,),
    ((240,  0,0,0,),    (255,255,255,255,), IDX_E,),
    (( 10,  0,0,0,),    ( 10,255,255,255,), IDX_PRIVATE,),
    ((172, 16,0,0,),    (172, 31,255,255,), IDX_PRIVATE,),
    ((192,168,0,0,),    (192,168,255,255,), IDX_PRIVATE,),
]
...
def get_ip_class(addr):
    try:
        addr_list = map(int, addr.split("."))
    except:
        return False

    head = addr_list[0]
    if head == 0 or head == 127:    # need skip
        return True
    
    result = []
    for node in MAP_CLASS:
        zs, ze, idx = node
        isChecked = True
        
        for i in xrange(4):
            if addr_list[i] < zs[i] or addr_list[i] > ze[i]:
                isChecked = False
                break
...
def check_mask(mask):
    if mask in INVALID_MASK:
        return False

    num_mask = addr2dec(mask)
    if num_mask is None:
        return False

    # head 1 left 0 check
    if (((~num_mask) & 0xFFFFFFFF)>>1) & num_mask > 0:
        return False
    return True
...
try:
    ...
    while True:
        ...
        if line == "":
            break
        ip_addr, mask = line.split("~")

        ip_class = get_ip_class(ip_addr)
        # print ip_class
        
        if not check_mask(mask) or not ip_class:
            # print "invalid mask"
            recorder[IDX_INVALID] += 1
            continue
